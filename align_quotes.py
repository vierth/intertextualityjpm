...
alignment_docs = ["KR2a0018 梁書-唐-姚思廉_10",
                  "KR2a0024 南史-唐-李延壽_54", "KR2a0018 梁書-唐-姚思廉_11"]
...
def align(quote1, quote2, params):
    matchscore = params["match_score"]
    misalignscore = params["misalign_score"]
    mismatchscore = params["mismatch_score"]
    chunklim = params["chunk_limit"]
...
def runalignment(content, totallength, params):
    global tracker
    info = content.split("\t")
    # print(info)
    ...
    if float(info[3]) != 1.0:
        ...
        aligneda, alignedb = align(info[6], info[7], params)
...
def run(cfg):
    ...
    with open(cfg["files"]["input_file"], "r",encoding="utf8") as rf:
        contents = rf.read().split("\n")
        contents = contents[1:]
        contents = [c for c in contents if c != ""]

    # check to see if there are any results in the file
    if len(contents) == 0:
        print("no results in file")
        return

    # If alignment_docs have beeen provided, extract the relevant quotes
    if cfg["params"]["alignment_docs"]:
        use_contents = []
        pairs = set()
        for t1 in cfg["params"]["alignment_docs"]:
            for t2 in cfg["params"]["alignment_docs"]:
                if t1 != t2:
                    pairs.add((t1, t2))
    ...
    results = pool.starmap(runalignment, zip(
        contents, repeat(len(contents)), repeat(cfg["params"])))
...
if __name__ == "__main__":
    cfg = read_config_file("config/align_config.json")
    run(cfg)
